...
import { runConnectorMirrorScenario } from './helpers/app-connector-mirror-scenario';
...
describe('Inflight backend e2e', () => {
  ...
  it('issues valid service-account tokens, rejects invalid audience and revokes stale tokens after rotation', async () => {
    ...
    try {
      ...
      await context.http
        .post('/api/v1/service-accounts/not-a-mongo-id/rotate-credential')
        .set('Authorization', `Bearer ${ownerAccessToken}`)
        .expect(404);
      ...
      await context.models.serviceCredentials.updateOne(
      ...
      await context.models.serviceAccounts.updateOne(
        { _id: serviceAccountResponse.body.id as string },
  ...
  it('mirrors project connector connections with local projectKey validation, redaction and sync', async () => {
    await runConnectorMirrorScenario();
  });
});
